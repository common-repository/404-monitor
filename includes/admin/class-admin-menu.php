...
class Admin_Menu implements Runner {
	...
	public function register_pages() {
		...
		new Page( 'rank-math-monitor', esc_html__( 'Rank Math', '404-monitor' ), array(
			'position'   => 80,
		...
		// Help & Support.
		new Page( 'rank-math-help', esc_html__( 'Help &amp; Support', '404-monitor' ), array(
			'position'   => 99,
	...
	public function fix_first_submenu() {
		global $submenu;
		if ( ! isset( $submenu['rank-math-monitor'] ) ) {
			return;
		}

		if ( current_user_can( 'manage_options' ) && 'Rank Math' === $submenu['rank-math-monitor'][0][0] ) {
			$submenu['rank-math-monitor'][0][0] = esc_html__( 'Dashboard', '404-monitor' );
		} else {
			unset( $submenu['rank-math-monitor'][0] );
		}

		if ( empty( $submenu['rank-math-monitor'] ) ) {
			return;
		}

		// Store ID of first_menu item so we can use it in the Admin menu item.
		set_transient( 'rank_math_first_submenu_id', array_values( $submenu['rank-math-monitor'] )[0][2] );
	}
