...
use RankMath_Monitor\Replace_Vars;
...
class Options {
	...
	function register_option_page() {
		...
		foreach ( $tabs as $id => $tab ) {
			...
			$cmb->add_field( array(
				'name' => esc_html__( 'Panel', '404-monitor' ),
				'id'   => 'setting-panel-' . $id,
				'type' => 'tab_open',
			) );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'font-awesome', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css', null, rank_math_monitor()->version );
		wp_enqueue_style( 'rank-math-options', rank_math_monitor()->plugin_url() . 'assets/admin/css/option-panel.css', array( 'rank-math-common', 'rank-math-cmb2' ), rank_math_monitor()->version );
		wp_enqueue_script( 'rank-math-options', rank_math_monitor()->plugin_url() . 'assets/admin/js/option-panel.js', array( 'underscore', 'rank-math-common' ), rank_math_monitor()->version, true );

		// Add thank you.
		GlobalHelper::add_json( 'indexUrl', rank_math_monitor()->plugin_url() . 'assets/admin/js/search-index/' );
	...
	private function locate_file( $id, $tab ) {
		...
		$file = isset( $tab['file'] ) && ! empty( $tab['file'] ) ? $tab['file'] : rank_math_monitor()->includes_dir() . "settings/{$this->folder}/{$id}.php";
