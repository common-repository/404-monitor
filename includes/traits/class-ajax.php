 ...
 * @since      1.0.0
 * @package    RankMath
 * @subpackage RankMath_Monitor\Traits
 * @author     Rank Math <support@rankmath.com>
 */

namespace RankMath_Monitor\Traits;
...
trait Ajax {
	...
	protected function ajax( $tag, $function_to_add, $priority = 10 ) {
		\add_action( 'wp_ajax_rank_math_monitor_' . $tag, [ $this, $function_to_add ], $priority );
	}
	...
	public function verify_nonce( $action ) {
		if ( ! isset( $_REQUEST['security'] ) || ! \wp_verify_nonce( $_REQUEST['security'], $action ) ) {
			$this->error( __( 'Error: Nonce verification failed', '404-monitor' ) );
		}
	}
	...
	public function has_cap_ajax( $capability ) {
		...
		if ( ! Helper::has_cap( $capability ) ) {
			$this->error( esc_html__( 'You are not authorized to perform this action.', '404-monitor' ) );
		}
